...
public class SwerveDriveTest {
    ...
    public static double findCouplingRatio(SwerveDrive swerveDrive, double volts, boolean automatic) {
        ...
        for (SwerveModule module : swerveDrive.getModules()) {
            ...
            double driveEncoderPositionRotations = module.getDriveMotor().getPosition() / module.configuration.conversionFactors.drive;
            if (automatic) {
                ...
                DriverStation.reportWarning("Spin the " + module.configuration.name + " module 360 degrees now, you have 1 minute.\n", false);
                Timer.delay(60);
            }
            double couplingRatio = (module.getDriveMotor().getPosition() / module.configuration.conversionFactors.drive) - driveEncoderPositionRotations;
    ...
    public static void logDriveMotorActivity(SwerveModule module, SysIdRoutineLog log, Supplier<Double> powerSupplied) {
        ...
        log.motor("drive-" + module.configuration.name).voltage(m_appliedVoltage.mut_replace(power, Volts)).linearPosition(m_distance.mut_replace(distance, Meters)).linearVelocity(m_velocity.mut_replace(velocity, MetersPerSecond));
    }
     ...
     */
    public static SysIdRoutine setDriveSysIdRoutine(Config config, SubsystemBase swerveSubsystem, SwerveDrive swerveDrive, double maxVolts) {
        return new SysIdRoutine(config, new SysIdRoutine.Mechanism((Measure<Voltage> voltage) -> {
    ...
    public static void logAngularMotorActivity(SwerveModule module, SysIdRoutineLog log, Supplier<Double> powerSupplied) {
        ...
        log.motor("angle-" + module.configuration.name).voltage(m_appliedVoltage.mut_replace(power, Volts)).angularPosition(m_anglePosition.mut_replace(angle, Degrees)).angularVelocity(m_angVelocity.mut_replace(velocity, DegreesPerSecond));
    }
     ...
     */
    public static SysIdRoutine setAngleSysIdRoutine(Config config, SubsystemBase swerveSubsystem, SwerveDrive swerveDrive) {
        return new SysIdRoutine(config, new SysIdRoutine.Mechanism((Measure<Voltage> voltage) -> {
