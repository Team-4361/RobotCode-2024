...
public class SwerveDrive {
     ...
     */
    private final Alert tunerXRecommendation = new Alert("Swerve Drive", "Your Swerve Drive is compatible with Tuner X swerve generator, please consider using that instead of YAGSL. More information here!\n" + "https://pro.docs.ctr-electronics.com/en/latest/docs/tuner/tuner-swerve/index.html", AlertType.WARNING);
    /**
     * Field object.
     */
     ...
     */
    public SwerveDrive(SwerveDriveConfiguration config, SwerveControllerConfiguration controllerConfig, double maxSpeedMPS) {
        ...
        swerveDrivePoseEstimator = new SwerveDrivePoseEstimator(kinematics, getYaw(), getModulePositions(), new Pose2d(new Translation2d(0, 0), Rotation2d.fromDegrees(0))); // x,y,heading in radians; Vision measurement std dev, higher=less weight
        ...
        if (SwerveDriveTelemetry.verbosity.ordinal() >= TelemetryVerbosity.HIGH.ordinal()) {
            ...
            SwerveDriveTelemetry.sizeFrontBack = Units.metersToInches(SwerveMath.getSwerveModule(swerveModules, true, false).moduleLocation.getX() + SwerveMath.getSwerveModule(swerveModules, false, false).moduleLocation.getX());
            SwerveDriveTelemetry.sizeLeftRight = Units.metersToInches(SwerveMath.getSwerveModule(swerveModules, false, true).moduleLocation.getY() + SwerveMath.getSwerveModule(swerveModules, false, false).moduleLocation.getY());
    ...
    public void drive(Translation2d translation, double rotation, boolean fieldRelative, boolean isOpenLoop, Translation2d centerOfRotationMeters) {
        ...
        // necessary.
        ChassisSpeeds velocity = fieldRelative ? ChassisSpeeds.fromFieldRelativeSpeeds(translation.getX(), translation.getY(), rotation, getOdometryHeading()) : new ChassisSpeeds(translation.getX(), translation.getY(), rotation);
    ...
    public void drive(ChassisSpeeds velocity, boolean isOpenLoop, Translation2d centerOfRotationMeters) {
        ...
        if (headingCorrection) {
            if (Math.abs(velocity.omegaRadiansPerSecond) < HEADING_CORRECTION_DEADBAND && (Math.abs(velocity.vxMetersPerSecond) > HEADING_CORRECTION_DEADBAND || Math.abs(velocity.vyMetersPerSecond) > HEADING_CORRECTION_DEADBAND)) {
                velocity.omegaRadiansPerSecond = swerveController.headingCalculate(getOdometryHeading().getRadians(), lastHeadingRadians);
            } else {
    ...
    private void setRawModuleStates(SwerveModuleState[] desiredStates, boolean isOpenLoop) {
        ...
        if (attainableMaxTranslationalSpeedMetersPerSecond != 0 || attainableMaxRotationalVelocityRadiansPerSecond != 0) {
            SwerveDriveKinematics.desaturateWheelSpeeds(desiredStates, getRobotVelocity(), maxSpeedMPS, attainableMaxTranslationalSpeedMetersPerSecond, attainableMaxRotationalVelocityRadiansPerSecond);
        } else {
    ...
    public ChassisSpeeds getFieldVelocity() {
        ...
        return ChassisSpeeds.fromFieldRelativeSpeeds(kinematics.toChassisSpeeds(getStates()), getOdometryHeading().unaryMinus());
    }
    ...
    public void setMaximumSpeed(double maximumSpeed, boolean updateModuleFeedforward, double optimalVoltage) {
        ...
        for (SwerveModule module : swerveModules) {
            ...
            if (updateModuleFeedforward) {
                module.setFeedforward(SwerveMath.createDriveFeedforward(optimalVoltage, maximumSpeed, swerveDriveConfiguration.physicalCharacteristics.wheelGripCoefficientOfFriction));
            }
        }
    }
